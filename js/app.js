(() => {
    ...
    function initSliders() {
        if (document.querySelector(".swiper")) new swiper_core_Swiper(".swiper", {
            ...
            speed: 800,
            resistanceRatio: 0,
            //!За необхідності призначається, якщо треба керувати кількісттю пагінаційних елементів
            scrollbar: {
                el: ".swiper-scrollbar",
                draggable: true
            },
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        const itemArea = document.querySelectorAll(".body-cell__item-inner");
        const showMoreBtn = document.querySelector(".block__more");
        const container = document.querySelector(".block__content");
        const blockDecore = document.querySelector(".block__decore");
        if (!container || !showMoreBtn || itemArea.length === 0 || !blockDecore) return;
        const getItemsToShow = width => breakpoints.find((bp => width >= bp.width))?.itemsToShow || 1;
        const getItemsToShowOnClick = width => breakpoints.find((bp => width >= bp.width))?.itemsToShowOnClick || 1;
        let itemsToShow = getItemsToShow(window.innerWidth);
        let itemsShown = itemsToShow;
        const enableScale = false;
        function initializeItems() {
            itemArea.forEach(((item, index) => {
                if (index < itemsShown) {
                    item.style.display = "flex";
                    item.style.opacity = "1";
                    item.style.transform = enableScale ? "scale(1)" : "none";
                } else {
                    item.style.display = "none";
                    item.style.opacity = "0";
                    item.style.transform = enableScale ? "scale(0.6)" : "none";
                }
            }));
            updateButtonText();
        }
        function toggleItems(applyAnimation = true) {
            itemArea.forEach(((item, index) => {
                if (index < itemsShown) {
                    if (item.style.display !== "flex") {
                        item.style.display = "flex";
                        item.style.setProperty("will-change", "opacity, transform");
                        if (applyAnimation) setTimeout((() => {
        ...
        function updateButtonText() {
            const isExpanded = itemsShown >= itemArea.length;
            showMoreBtn.textContent = isExpanded ? "Collapse" : "Expand";
            if (isExpanded) showMoreBtn.classList.add("block__more--expanded"); else showMoreBtn.classList.remove("block__more--expanded");
            blockDecore.style.transition = "opacity 0.3s ease";
            blockDecore.style.opacity = isExpanded ? "0" : "1";
        }
    ...
    //!Исчезновение кнопки
        ...
        const button = document.getElementById("btn-function");
        let lastScrollTop = 0;
        window.addEventListener("scroll", (() => {
            ...
            if (scrollTop > lastScrollTop && scrollBottom <= documentHeight * .08) {
            ...
            } else if (scrollTop < lastScrollTop && scrollBottom > documentHeight * .08) if (button.classList.contains("hidden")) {
